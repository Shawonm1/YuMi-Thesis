function [positions, velocities, accelerations, timestamp, success] = exampleHelperPlanExecuteTrajectoryPickPlace(robotL, mpcTimeStep, obstacles, endEffector, jointInit, taskFinal, tolerance, avoidCollisions)
...
curFormat = robotL.DataFormat;
robotL.DataFormat = 'column';

% Disable display messages
mpcverbosity off;

% Get number of joints
numJoints = numel(robotL.homeConfiguration);
...
if ~isempty(obstacles) && avoidCollisions
    [~, separationDist, ~] = checkCollision(robotL, homeConfiguration(robotL), obstacles, 'IgnoreSelfCollision', 'On', 'Exhaustive', 'on');
    tempDistances = separationDist(1:robotL.NumBodies,1:numObstacles);
    bodyIndices = find((isinf(tempDistances(:,1))|isnan(tempDistances(:,1)))==0);
    numBodies = numel(bodyIndices);
else
    numBodies = robotL.NumBodies;
end
...
nlobj.Optimization.CustomCostFcn = @(X,U,e,data) nlmpcCostFunction(X,U,e,data, poseFinal, robotL, endEffector, Qr, Qt, Qu, Qv); 

if ~isempty(world) && avoidCollisions
    nlobj.Optimization.CustomIneqConFcn = @(X,U,e,data) myIneqConFunction(X,U,e,data, safetyDistance, world, robotL);
end
...
nlobj.Jacobian.CustomCostFcn = @(X,U,e,data) nlmpcJacobianCost(X,U,e,data, poseFinal, robotL, endEffector, Qr, Qt, Qu, Qv);

if ~isempty(world) && avoidCollisions
    nlobj.Jacobian.CustomIneqConFcn = @(X,U,e,data) nlmpcJacobianConstraint(X,U,e,data, world, robotL);
end
...
nlobj.States = struct('Min',stateMinValues,...
    'Max',stateMaxValues);
nlobj.MV = struct('Min',{-1;-1;-1;-1;-1;-10;-10;-10;-10},'Max',{1;1;1;1;1;10;10;10;10});
...
for timestep=1:maxIters
    ...
    taskTempFinal = getTransform(robotL, jointTempFinal', endEffector);
